                                ...
                                unsigned int sample_rate)
{
    ...
    max_dosr = (AIC32X4_MAX_DOSR_FREQ / sample_rate / dosr_increment) * dosr_increment;
    min_dosr = (AIC32X4_MIN_DOSR_FREQ / sample_rate / dosr_increment) * dosr_increment;
    ...
    for(nadc = max_nadc; nadc > 0; --nadc) {
        ...
        for(dosr = max_dosr; dosr >= min_dosr; dosr -= dosr_increment) {
            ...
            for(mdac = min_mdac; mdac <= 128; ++mdac) {
                for(ndac = max_ndac; ndac > 0; --ndac) {
                    ...
                    if(dac_clock_rate == adc_clock_rate) {
                        ...
                        clk_set_rate(clocks[5].clk, sample_rate * 32);
                        return 0;
                    }
                }
            }
