...
static int clk_aic32x4_pll_get_muldiv(struct clk_aic32x4 *pll, struct clk_aic32x4_pll_muldiv *settings)
{
...
static int clk_aic32x4_pll_set_muldiv(struct clk_aic32x4 *pll, struct clk_aic32x4_pll_muldiv *settings)
{
	int ret;
	/*	Change to use regmap_bulk_write for some if not all? */

	ret = regmap_update_bits(pll->regmap, AIC32X4_PLLPR, AIC32X4_PLL_R_MASK, settings->r);
	if(ret < 0)
		return ret;

	ret = regmap_update_bits(pll->regmap, AIC32X4_PLLPR, AIC32X4_PLL_P_MASK, settings->p << AIC32X4_PLL_P_SHIFT);
	if(ret < 0)
		return ret;
...
static unsigned long clk_aic32x4_pll_calc_rate(struct clk_aic32x4_pll_muldiv *settings, unsigned long parent_rate)
{
	u64 rate;
	/*
	 * We scale j by 10000 to account for the decimal part of P and divide it back
	 * out later.
	 */
...
static int clk_aic32x4_pll_calc_muldiv(struct clk_aic32x4_pll_muldiv *settings, unsigned long rate, unsigned long parent_rate)
{
	u64 multiplier;
	...
	/*
	 * We scale this figure by 10000 so that we can get the decimal part of the
	 * multiplier.	This is because we can't do floating point math in the kernel.
	 */
...
static unsigned long clk_aic32x4_pll_recalc_rate(struct clk_hw *hw, unsigned long parent_rate)
{
	struct clk_aic32x4 *pll = to_clk_aic32x4(hw);
...
static long clk_aic32x4_pll_round_rate(struct clk_hw *hw, unsigned long rate, unsigned long *parent_rate)
{
	struct clk_aic32x4_pll_muldiv settings;
	int ret;
...
static int clk_aic32x4_pll_set_rate(struct clk_hw *hw, unsigned long rate, unsigned long parent_rate) {
	struct clk_aic32x4 *pll = to_clk_aic32x4(hw);
	struct clk_aic32x4_pll_muldiv settings;
	int ret;
...
static int clk_aic32x4_pll_set_parent(struct clk_hw *hw, u8 index) {
	...
	return regmap_update_bits(pll->regmap, AIC32X4_CLKMUX, AIC32X4_PLL_CLKIN_MASK, index << AIC32X4_PLL_CLKIN_SHIFT);
}
...
static int clk_aic32x4_codec_clkin_set_parent(struct clk_hw *hw, u8 index) {
    ...
    return regmap_update_bits(mux->regmap, AIC32X4_CLKMUX, AIC32X4_CODEC_CLKIN_MASK, index << AIC32X4_CODEC_CLKIN_SHIFT);
}
...
static int clk_aic32x4_div_set_rate(struct clk_hw *hw, unsigned long rate, unsigned long parent_rate)
{
    struct clk_aic32x4 *div = to_clk_aic32x4(hw);
    u8 divisor;
    ...
    return regmap_update_bits(div->regmap, div->reg, AIC32X4_DIV_MASK, divisor);
}

static long clk_aic32x4_div_round_rate(struct clk_hw *hw, unsigned long rate, unsigned long *parent_rate)
{
...
static unsigned long clk_aic32x4_div_recalc_rate(struct clk_hw *hw, unsigned long parent_rate)
{
    struct clk_aic32x4 *div = to_clk_aic32x4(hw);
...
static int clk_aic32x4_bdiv_set_parent(struct clk_hw *hw, u8 index)
{
    ...
    return regmap_update_bits(mux->regmap, AIC32X4_IFACE3, AIC32X4_BDIVCLK_MASK, index);
}
...
static struct aic32x4_clkdesc aic32x4_clkdesc_array [] = {
	{
		.name = "pll",
		.parent_names = (const char* []) { "mclk", "bclk", "gpio", "din" },
		.num_parents = 4,
		.ops = &aic32x4_pll_ops,
		.reg = 0,
	},
    {
        .name = "codec_clkin",
        .parent_names = (const char *[]) { "mclk", "bclk", "gpio", "pll" },
        .num_parents = 4,
    ...
    },
    {
        ...
        {
        .name = "bdiv",
        .parent_names = (const char *[]) { "ndac", "mdac", "nadc", "madc" },
        .num_parents = 4,
...
static struct clk *aic32x4_register_clk(struct device *dev, struct aic32x4_clkdesc *desc)
{
...
int aic32x4_register_clocks(struct device *dev, const char *mclk_name)
{
	...
	/*
	 * These lines are here to preserve the current functionality of the driver with
	 * regard to the DT.  These should eventually be set by DT nodes so that the
	 * connections can be set up in configuration rather than code.
	 */
